...
const createCommunity = asyncHandler(async (req, res) => {
    ...
    if (communityImage) {
        ...
        const s3Response = await uploadFileS3(communityImageLocalPath, `communityId-${newCommunity._id}${path.extname(communityImage.filename)}`);
...
const updateCommunity = asyncHandler(async (req, res) => {
    ...
    if (communityImage) {
        ...
        if (community[0].image !== "") {
            await deleteS3PublicImageFile((community[0].image).replace(`https://s3.${process.env.AWS_REGION}.amazonaws.com/${process.env.AWS_S3_PUBLIC_IMAGE_BUCKET_NAME}/`, '') || "");
        }
    ...
    await Community.findByIdAndUpdate(communityId, { text: text }, { new: true });
...
const deleteCommunity = asyncHandler(async (req, res) => {
    ...
    if (community.image) {
        await deleteS3PublicImageFile((community.image).replace(`https://s3.${process.env.AWS_REGION}.amazonaws.com/${process.env.AWS_S3_PUBLIC_IMAGE_BUCKET_NAME}/`, '') || "");
    }
...
const getChannelCommunity = asyncHandler(async (req, res) => {
    ...
    const {
        channelId = "",
        page = 1,
        limit = 10 } = req.query; // Extract pagination parameters from the query
